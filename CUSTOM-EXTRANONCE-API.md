...
2. **Stratum core** — the process your miner actually connects to. It refreshes
   ...
   When your miner opens a channel, the core puts your prefix straight into the
   `OpenExtendedMiningChannel.Success` it answers with — the field every SV2
   client reads to learn where its rollable extranonce region starts. For a
   change while the miner is already connected it sends an SV2
   `SetExtranoncePrefix` instead (see [When the EN is used](#when-the-en-is-used)).
...
Your address page carries it: `GET /api/client/<address>` lists every worker
with a live session, and each entry now has an `extranonce` field — the stored
prefix as 8 hex chars, or `null` when the worker runs on the pool-allocated
one. No token needed; it is the same public address view the dashboard uses.

```jsonc
// GET /api/client/bc1q… (excerpt)
{
  "workers": [
    { "name": "rig1", "extranonce": "c0debabe", "hashRate": 1.2e13, … },
    { "name": "rig2", "extranonce": null,       "hashRate": 9.8e12, … }
  ]
}
```

**What this tells you, and what it doesn't.** It is the stored configuration:
use it to confirm a batch landed and to see which workers carry an override. It
is **not** a statement that a prefix is in effect on a running miner — that
depends on the core's Solo / Extended / primary-channel conditions in a
different process (see [Current limitations](#current-limitations)). Your miner
receives the value it actually got in the SV2
`OpenExtendedMiningChannel.Success` (and, on a live change,
`SetExtranoncePrefix`); that is the authoritative "what am I hashing with"
answer today.

Workers you never set are absent from the override set entirely, and a worker
with no live session does not appear in this view at all — it only lists
connected workers.
...
- **At channel-open — always works.** The override applies when your miner opens
  an Extended channel, i.e. when it connects or reconnects, and it travels in
  the channel-open success itself. No optional message is involved, so this path
  works with any SV2 client. Set the override **before** the miner connects and
  it is baked into the first job.
- **Live, without reconnect — only if your firmware implements
  `SetExtranoncePrefix`.** SV2 has exactly one message for changing a prefix
  mid-session (message type `0x19`), and a miner that does not implement it
  silently keeps the prefix it was opened with. It then hashes a coinbase the
  pool no longer rebuilds, and **every** share is rejected until it reconnects.
  Known not to implement it: NerdQAxe SV2 firmware (its frame dispatch handles
  six message types and this is not one of them). If in doubt, treat a change as
  needing a reconnect and restart the miner after setting it.
- **Caveat — the first override needs a connect.** If your miner is already
  ...
  until the miner reconnects. In practice: set your initial override before
  starting the miner.
...
- **Live changes depend on the firmware** implementing `SetExtranoncePrefix`
  (see [When the EN is used](#when-the-en-is-used)). Channel-open does not.
...
3b. GET /api/client/<address>
        → the `extranonce` field on each worker shows what is stored
