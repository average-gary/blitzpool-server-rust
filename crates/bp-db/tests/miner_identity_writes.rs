...
//! `0016_add_miner_identity.sql` must be applied or every test here fails on
...
async fn a_rotating_identity_round_trips_with_no_address() {
    ...
    // number moved once already (62 → 90, `0017_widen_identity_columns.sql`) and
...
///
/// The backfill in `0016_add_miner_identity.sql` copies `pplns_balance` only, so
