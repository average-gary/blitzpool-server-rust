...
///    `0017_widen_identity_columns.sql` widened the columns, as its own change
...
/// (`0017_widen_identity_columns.sql`), so hex would fit now. The tripwire that
...
impl RotatingPayout {
    ...
    /// `0017_widen_identity_columns.sql`, and a `payout_id` is 47 characters:
...
mod tests {
    ...
    /// (`0017_widen_identity_columns.sql`), the control fired, and the revisiting
    ...
    /// (`0017_widen_identity_columns.sql`, `MAX_ADDRESS_LEN = 90`), so the same
